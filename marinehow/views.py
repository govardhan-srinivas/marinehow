...
def profile(request, usuario_id=None):
    if usuario_id is None:
        try:
            ...
            return render(request, 'marinehow/please_login.html')
    ...
    return render(request, 'marinehow/profile.html', context)
...
#     return render(request, 'marinehow/question_list.html', context)
...
def auth_view(request):
    ...
    if user is not None:
        if user.is_active:
            ...
            return HttpResponseRedirect(reverse("marinehow:my_profile"))
...
def logout_user(request):
        auth.logout(request)
        return redirect(reverse('marinehow:questions'))
...
def question_detail(request, question_id):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            answer.save()
            return HttpResponseRedirect(reverse('marinehow:question', args=(question_id,)))
        else:
            return HttpResponse("{}\n<a href='{}'>Back to Question".format(
                form.errors, reverse('marinehow:question', args=(
                    question_id,))))
    else:
    ...
    return render(request, 'marinehow/question.html', context)
...
def new_question(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question.save()
            return HttpResponseRedirect(reverse('marinehow:question', args=(question.id,)))
        else:
            return HttpResponse("{}\n<a href='{}'>Back to New Question".format(
                form.errors, reverse('marinehow:new_question')))
    else:
        form = QuestionForm()
        context = {'form': form}
        return render(request, 'marinehow/new_question.html', context)
...
def new_comment(request, parent_type, parent_id):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('marinehow:question',
                                                args=(question_id,)))
        else:
            return HttpResponse("{}\n<a href='{}'>Back to New Comment".format(
                form.errors, reverse('marinehow:new_comment', args=(
        ...
        return render(request, 'marinehow/new_comment.html', context)
...
def vote(request):
    if request.method == "POST":
        ...
        marinehow_config = apps.get_app_config("marinehow")
        votable = marinehow_config.get_model(model_name).objects.get(id=vote_id)
...
def register_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('marinehow:register_success'))
        else:
    ...
    return render_to_response('marinehow/register.html', args)
